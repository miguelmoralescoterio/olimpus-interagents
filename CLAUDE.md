...
This file provides guidance to AI agent (Claude Code) when working with code in this repository.

## What this is

An agent-to-agent messaging bus for AI agent: multiple agent sessions on
...
- **Plugin** (recommended): `/plugin marketplace add …` → `/plugin
  install interagents@interagents`, or `claude --plugin-dir <repo>`
  for local dev. Adds `userConfig` (port, idle-shutdown) and
  `monitors.json`. User invokes as `/interagents:interagents …`.
- **Standalone skill**: clone or symlink `skills/interagents/` to
  `~/.claude/skills/interagents/` (e.g. `ln -s
  <repo>/skills/interagents ~/.claude/skills/interagents`). The
  skill is self-contained — `bin/`, `requirements.txt`, and `SKILL.md`
  all live inside `skills/interagents/`, so a copy or symlink of just
  that subdirectory is a fully working skill. User invokes as
  `/interagents …` (no plugin namespace). No `userConfig`; override
  defaults via `INTERAGENTS_PORT` / `INTERAGENTS_IDLE_MINUTES` env
  vars if needed.

Codex also has a repo-local plugin under `plugins/interagents-codex/`
with its marketplace entry in `.agents/plugins/marketplace.json`.
That bundle includes the same runtime CLI under
`plugins/interagents-codex/skills/interagents/bin/` and is the source
for Codex UI installation.

The skill content (`skills/interagents/SKILL.md`) is install-mode
...
and the new spawn exits with `[interagents] another monitor for this
...
pattern that the current Claude plugin schema requires (`"skills": ["./"]`
is rejected with "Path escapes plugin directory"). **`bin/` lives
inside the skill dir** (`skills/interagents/bin/`). The plugin's
monitor `when` defaults to `on-skill-invoke:interagents` (lazy);
`bin/auto_start.py` flips it to `always` when the user runs
`/interagents auto-start on`. Empirically `on-skill-invoke` may not
reliably auto-spawn a working monitor in current Claude Code versions, so the
...
`skills/interagents/bin/<script>.py`.
...
`skills/interagents/requirements.txt` (websockets + psutil); dev
...
2. **`bin/client.py`** — long-lived per-session monitor. Each stdout
   line becomes a AI agent notification. Manages reconnect with
...
| Stdout notification body       | 400 chars (issue #2: AI agent clips each monitor notification at ~512 chars total, so the body cap is sized to leave room for our prefix) |
...
- **State isolation**: the `tmp_data_dir` fixture sets
  `INTERAGENTS_DATA_DIR` to a per-test temp path so the suite never
  touches `~/.olimpus/interagents/`.
...
- **PPID override**: subprocesses spawned in a single test share the
  ...
  `INTERAGENTS_PPID_OVERRIDE` to give each subprocess a distinct
  pseudo-ppid.
...
- **Don't blanket `pkill -f 'bin/(client|server).py'`** during local
  testing — it will kill real user interagents monitors running in
  other agent sessions. Target specific pids via the pidfile
  (`~/.olimpus/interagents/server.<port>.pid`) or
