...
BIN_DIR = REPO / "skills" / "interagents" / "bin"


@pytest.fixture
def tmp_data_dir(tmp_path, monkeypatch):
    d = tmp_path / "interagents"
    monkeypatch.setenv("INTERAGENTS_DATA_DIR", str(d))
    return d
...
def _spawn_client(port, name, env_data_dir, ppid_override=None, extra_env=None):
    env = os.environ.copy()
    env["INTERAGENTS_DATA_DIR"] = str(env_data_dir)
    env["PYTHONPATH"] = str(REPO)
    if ppid_override is not None:
        env["INTERAGENTS_PPID_OVERRIDE"] = str(ppid_override)
    if extra_env:
...
class TestEnsureServerRunning:
    ...
    def test_direct_bind_writes_identity_only_after_bind_succeeds(self, tmp_data_dir, free_port):
        ...
        try:
            env = os.environ.copy()
            env["INTERAGENTS_DATA_DIR"] = str(tmp_data_dir)
            env["PYTHONPATH"] = str(REPO)
            proc = subprocess.run(
...
class TestNameCollisionAutoRetry:
    ...
    def test_second_listener_auto_renames(self, tmp_data_dir, free_port):
        ...
        env["PYTHONPATH"] = str(REPO)
        env_a = env.copy()
        env_a["INTERAGENTS_PPID_OVERRIDE"] = "40001"
        ...
        env_b = env.copy()
        env_b["INTERAGENTS_PPID_OVERRIDE"] = "40002"
    ...
    def test_exhausted_retries_stops(self, tmp_data_dir, free_port):
        ...
        try:
            for i, key in enumerate(("50001", "50002", "50003")):
                env_i = env.copy()
                env_i["INTERAGENTS_PPID_OVERRIDE"] = key
                p = subprocess.Popen(
                    [sys.executable, "-u", str(BIN_DIR / "client.py"),
...
class TestReElectionAfterServerCrash:
    def test_client_respawns_server_after_kill(self, tmp_data_dir, free_port):
        ...
        # Spawn one client.
        env_c = env.copy()
        env_c["INTERAGENTS_PPID_OVERRIDE"] = "30001"
...
class TestClientIntegration:
    def test_two_clients_exchange_messages(self, tmp_data_dir, free_port):
        ...
        try:
            ...
            # Read beta's stdout for the interagents line
...
class TestEnvVarConfig:
    """Verify client.py picks up CLAUDE_PLUGIN_OPTION_* and INTERAGENTS_* env vars
    ...
    def test_plugin_option_port(self, monkeypatch):
        ...
        monkeypatch.delenv("INTERAGENTS_PORT", raising=False)
        assert _env_int("CLAUDE_PLUGIN_OPTION_PORT", "INTERAGENTS_PORT", default=9474) == 9499

    def test_inter_session_port_fallback(self, monkeypatch):
        from bin.client import _env_int
        monkeypatch.delenv("CLAUDE_PLUGIN_OPTION_PORT", raising=False)
        monkeypatch.setenv("INTERAGENTS_PORT", "9500")
        assert _env_int("CLAUDE_PLUGIN_OPTION_PORT", "INTERAGENTS_PORT", default=9474) == 9500

    def test_default_when_neither_set(self, monkeypatch):
        from bin.client import _env_int
        monkeypatch.delenv("CLAUDE_PLUGIN_OPTION_PORT", raising=False)
        monkeypatch.delenv("INTERAGENTS_PORT", raising=False)
        assert _env_int("CLAUDE_PLUGIN_OPTION_PORT", "INTERAGENTS_PORT", default=9474) == 9474

    def test_invalid_value_falls_through(self, monkeypatch):
        from bin.client import _env_int
        monkeypatch.setenv("CLAUDE_PLUGIN_OPTION_PORT", "not-a-port")
        monkeypatch.setenv("INTERAGENTS_PORT", "9501")
        assert _env_int("CLAUDE_PLUGIN_OPTION_PORT", "INTERAGENTS_PORT", default=9474) == 9501
