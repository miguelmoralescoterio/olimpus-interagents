...
SKILL = (REPO / "skills" / "interagents" / "SKILL.md").read_text()


class TestFrontmatter:
    def test_name(self):
        assert "name: interagents" in SKILL
...
class TestSubcommands:
    def test_dispatch_table_has_all_subcommands(self):
        for sub in ("connect", "install-deps", "list", "send", "broadcast",
                    "rename", "status", "disconnect", "drain"):
            assert f"`/interagents {sub}" in SKILL or f"`/interagents [args]" in SKILL
...
class TestInstallDepsUx:
    def test_uses_isolated_venv_by_default(self):
        ...
        assert "~/.olimpus/interagents/venv" in SKILL
...
class TestDedupGuard:
    def test_tasklist_check_in_connect(self):
        ...
        assert '"interagents messages"' in SKILL
