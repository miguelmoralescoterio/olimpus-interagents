...
class TestPaths:
    def test_data_dir_respects_env(self, tmp_path, monkeypatch):
        monkeypatch.setenv("INTERAGENTS_DATA_DIR", str(tmp_path / "x"))
        # Reload the module path resolver
        path = shared.data_dir()
        assert path == tmp_path / "x"

    def test_default_data_dir(self, monkeypatch):
        monkeypatch.delenv("INTERAGENTS_DATA_DIR", raising=False)
        path = shared.data_dir()
        assert str(path).endswith(".olimpus/interagents")
...
class TestResolveListenerKey:
    ...
    def test_falls_back_to_getppid_outside_cc(self, monkeypatch):
        monkeypatch.delenv("INTERAGENTS_PPID_OVERRIDE", raising=False)
    ...
    def test_invalid_override_falls_back(self, monkeypatch):
        monkeypatch.setenv("INTERAGENTS_PPID_OVERRIDE", "not-a-number")
        key = shared.resolve_listener_key()
        assert isinstance(key, int) and key > 0
...
class TestFindCcAncestorPidMatching:
    ...
    def test_foreground_symlinked_agent_still_matches(self, monkeypatch):
        """argv[0]='agent' (the ~/.local/bin/agent symlink path) continues
        to match. Guards against the bg-versioned-path patch accidentally
        regressing the common foreground case."""
        import psutil
        tree = {100: {"cmdline": ["agent", "--effort", "max"], "ppid": 1}}
    ...
    def test_background_versioned_path_returns_per_session_pid(self, monkeypatch):
        """Regression: in CC's background mode (Agent View / `agent --bg`)
        the per-session spare is launched by its resolved versioned path
        (e.g. ~/.local/share/agent/versions/2.1.146 --bg-spare), whose
        basename is a version number, not `agent`. The walk must recognize
        ...
        import psutil
        tree = {
            100: {"cmdline": ["/Users/x/.local/share/agent/versions/2.1.146",
                              "--bg-spare"], "ppid": 50},
            50:  {"cmdline": ["agent", "daemon", "run"], "ppid": 1},
        }
        monkeypatch.setattr(os, "getppid", lambda: 100)
        monkeypatch.setattr(psutil, "Process", self._fake_process_cls(tree))
        assert shared.find_cc_ancestor_pid() == 100
    ...
    def test_distinct_bg_sessions_resolve_to_distinct_pids(self, monkeypatch):
        ...
        otherwise the second `/interagents connect` exits with `another
        monitor for this session is already running`."""
        import psutil
        tree = {
            100: {"cmdline": ["/Users/x/.local/share/agent/versions/2.1.146",
                              "--bg-spare", "A"], "ppid": 50},
            200: {"cmdline": ["/Users/x/.local/share/agent/versions/2.1.146",
                              "--bg-spare", "B"], "ppid": 50},
            50:  {"cmdline": ["agent", "daemon", "run"], "ppid": 1},
        }
        monkeypatch.setattr(psutil, "Process", self._fake_process_cls(tree))
...
class TestVerifyServerIdentity:
    ...
    def test_no_pidfile_fails_closed(self, tmp_data_dir, monkeypatch):
        """The squatter scenario: no real server, just something on port 9474.
        Without a pidfile we can't verify; fail closed."""
        monkeypatch.setenv("INTERAGENTS_DATA_DIR", str(tmp_data_dir))
    ...
    def test_legacy_pidfile_must_match_host_too(self, tmp_data_dir, monkeypatch):
        ...
        monkeypatch.setenv("INTERAGENTS_DATA_DIR", str(tmp_data_dir))
        shared.secure_dir(tmp_data_dir)
        # Legacy: server.9474.pid only, no metadata, no host-scoped file.
        shared.pidfile_path(9474).write_text(f"{os.getpid()}\n")
        ...
        class FakeProcess:
            ...
            def cmdline(self):
                ...
                return ["/usr/bin/python", "/repo/bin/server.py", "--port", "9474"]
        ...
        assert shared.verify_server_identity("127.0.0.1", 9474)
        # …but MUST NOT validate a different host using the same pidfile
        assert not shared.verify_server_identity("evil.example", 9474)
        assert not shared.verify_server_identity("localhost", 9474)
