"""Long-lived per-session client. Stdout = notifications to AI agent."""
...
# system/user-level Python. Tests opt out via INTERAGENTS_NO_REEXEC=1.
import os
import sys
...
log = logging.getLogger("interagents.client")
...
def _format_truncation_pointer(msg_id: str, full_len: int) -> str:
    ...
    return f"[interagents msg={msg_id} cont] full text {full_len} bytes at {log_path}"
...
def _resolve_ppid() -> int:
    ...
    pid in real AI agent so helper CLIs (run from a *different* Bash
...
class Client:
    ...
    async def run(self) -> int:
        ...
        if self._lock_fd is None:
            ...
            if info:
                _print_line(
                    "[interagents] another monitor for this session is already running "
                    f"— name={info.get('name', '')!r}, "
                    f"listener_pid={info.get('listener_pid', '')}, "
                    f"session_id={info.get('session_id', '')}; exiting"
                )
            else:
                _print_line("[interagents] another monitor for this session is already running — exiting")
            return 0
        ...
        try:
            ...
            while not self._stop.is_set():
                ...
                try:
                    ...
                    _print_line(f"[interagents] connected to a non-interagents service on port {self.port}: {e}")
                    return 1
    ...
    async def _connect_and_serve(self) -> None:
        ...
        if not shared.verify_server_identity(self.host, self.port):
            _print_line(
                "[interagents] server identity check failed "
            ...
            if welcome.get("op") == "error":
                ...
                if code == shared.ErrorCode.NAME_TAKEN:
                    ...
                    if (candidates and
                        ...
                        _print_line(
                            f"[interagents] name {old_name!r} taken; "
                            f"using {new_name!r}"
                        )
                        return  # main loop will reconnect with self.name = new_name
                    # Out of retries — surface and stop. Caller picks a new name.
                    _print_line(
                        f"[interagents] name {self.name!r} taken after "
                        f"{self._collision_retries} retries; "
                        f"run /interagents connect <other-name>"
                    )
                    self._stop.set()
                    return
                _print_line(f"[interagents] hello rejected: {code} {welcome.get('message', '')}")
                self._stop.set()
                return
...
def main() -> int:
    ...
    #   3. INTERAGENTS_<KEY>         (manual env override)
    #   4. Hard-coded default
    parser = argparse.ArgumentParser()
    parser.add_argument("--host", default=os.environ.get("INTERAGENTS_HOST", "127.0.0.1"))
    parser.add_argument("--port", type=int, default=_env_int(
        "CLAUDE_PLUGIN_OPTION_PORT", "INTERAGENTS_PORT", default=shared.DEFAULT_PORT,
    ))
    parser.add_argument("--name", default=os.environ.get("INTERAGENTS_NAME", ""))
    parser.add_argument("--label", default=os.environ.get("INTERAGENTS_LABEL", ""))
    parser.add_argument("--idle-shutdown-minutes", type=float, default=_env_float(
        "CLAUDE_PLUGIN_OPTION_IDLE_SHUTDOWN_MINUTES", "INTERAGENTS_IDLE_MINUTES",
        default=10,
    ))
    ...
    if args.name and not shared.validate_name(args.name):
        _print_line(f"[interagents] invalid name {args.name!r}")
        return 1
    if not shared.validate_label(args.label):
        _print_line(f"[interagents] invalid label {args.label!r}")
        return 1

    # Plugin auto-start path: monitors.json doesn't pass --name (so the user
    # doesn't have to set INTERAGENTS_NAME). Fall back to a name derived from
    ...
    if not final_name:
        ...
        if final_name:
            auto_named = True
            _print_line(
                f"[interagents] no --name given; auto-named {final_name!r} "
                f"from cwd (rename with /interagents rename)"
            )
    ...
    try:
        ...
        _print_line(f"[interagents] dependencies missing — run /interagents install-deps ({e})")
        return 0
    finally:
        loop.close()
...
if __name__ == "__main__":
    try:
        ...
        _print_line("[interagents] dependencies missing — run /interagents install-deps")
        sys.exit(0)
    sys.exit(main())
