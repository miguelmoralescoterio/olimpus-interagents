...
# Body cap for the stdout notification line that AI agent's monitor
...
# our prefix (`[interagents msg=… from="…" "…" truncated=N] `) under
...
def data_dir() -> Path:
    env = os.environ.get("INTERAGENTS_DATA_DIR")
    if env:
        return Path(env)
    return Path.home() / ".olimpus" / "interagents"
...
def _identity_stem(port: int = DEFAULT_PORT, host: str | None = None) -> str:
    ...
    component so endpoints like localhost:9474 and 127.0.0.1:9474 do not
...
def verify_server_identity(host: str | None = None, port: int | None = None) -> bool:
    ...
    9474 first to harvest tokens from connecting clients).
...
def find_agent_ancestor_pid() -> int:
    """Walk up the process tree and return the closest supported agent pid.

    Claude Code, Codex, and Kiro commonly spawn tool commands from short-lived
    shells. The long-lived listener and helper CLIs are siblings, so keying
    listener state by a stable agent ancestor lets helpers find their own
    session without registering as visible peers.
    """
    try:
        import psutil
    except ImportError:
    ...
    while pid and pid not in seen and pid != 1:
        ...
        if cmd:
            ...
            if _looks_like_agent_process(exe, cmd):
                return pid
        try:
            pid = p.ppid()
        except (psutil.NoSuchProcess, psutil.AccessDenied):
...
def _looks_like_agent_process(exe: str, cmd: list[str]) -> bool:
    if exe in {"claude", "codex", "kiro", "agent"}:
        return True
    if exe.startswith(("claude-", "codex-", "kiro-", "agent-")):
        return True

    full = cmd[0].lower() if cmd else ""
    versioned_paths = (
        "/claude/versions/",
        "/.local/share/claude/",
        "/codex/versions/",
    ...
    if any(marker in full for marker in versioned_paths):
        return True
...
def resolve_listener_key() -> int:
    ...
    Resolution order:
      1. `INTERAGENTS_PPID_OVERRIDE` (test/debug)
      2. The nearest supported agent process pid.
      3. `os.getppid()` (fallback for plain terminal environments)
    """
    override = os.environ.get("INTERAGENTS_PPID_OVERRIDE")
    if override:
        try:
            return int(override)
        except ValueError:
            pass
    agent = find_agent_ancestor_pid()
    if agent > 0:
        return agent
    return os.getppid()
