...
name: interagents
description: |
  Agent-to-agent messaging bus for AI agent. Use this skill whenever the
  user wants to send messages between local AI agent sessions, delegate a
  task to another running session, fan-out work across multiple AI agent
  ...
  machine. Triggers include: "/interagents", "interagents", "connect to
  other sessions", "send message to another agent session", "list agent
  sessions", "broadcast", "delegate to another session", "tell other agent
...
Agent-to-agent messaging for AI agent sessions on the same machine.
...
`/interagents` invocation and substitute the absolute path into every
...
`<text>` is a message from a peer AI agent (another AI agent session).
...
Incoming notification:
  [interagents msg=q7r8 from="auth-refactor"] run pytest tests/test_auth.py
...
When the user invokes `/interagents [args]`, parse `args` to dispatch:

| User input                                    | Action                                                            |
| :-------------------------------------------- | :---------------------------------------------------------------- |
| `/interagents [connect]` (no name)          | Connect; auto-named (see connect section).                        |
| `/interagents connect <name>`               | Connect with the given ASCII name.                                |
| `/interagents install-deps`                 | Install runtime deps (websockets, psutil) with user confirmation. |
| `/interagents list`                         | Show connected sessions.                                          |
| `/interagents send <name-or-prefix> <text>` | Send to one peer.                                                 |
...
the new spawn exits cleanly with `[interagents] another monitor for
...
(`/interagents:interagents`) or standalone (`/interagents`,
`~/.claude/skills/interagents/SKILL.md`).
...
2. **Start the monitor**:
   ...
   3. `INTERAGENTS_PORT` / `INTERAGENTS_IDLE_MINUTES` (manual override)
   4. Defaults: `9474`, `10` minutes
...
3. **If the spawn returns
   `[interagents] another monitor for this session is already running — name='<existing>', listener_pid=<pid>, session_id=<id>; exiting`**:
   ...
   - **User did NOT supply a name** (typed just `/interagents:interagents`
...
**On `[interagents] name '…' taken; using '…-2'`**: informational only —
...
in your reply (e.g., "Connected as `interagents-dev-2` — `interagents-dev`
was already taken").

**On `[interagents] name '…' taken after N retries`**: the auto-retry budget
is exhausted (very rare; means many sessions in the same cwd). Tell the user
and ask them for a name: `/interagents connect <some-other-name>`.
...
`~/.olimpus/interagents/venv` so it never touches the user's
...
3. **Create the venv** if it doesn't already exist:
   - With uv: `uv venv ~/.olimpus/interagents/venv`
   - Without uv: `python3 -m venv ~/.olimpus/interagents/venv`
4. **Install runtime deps into the venv**:
   - With uv: `uv pip install -p ~/.olimpus/interagents/venv -r <bin>/../requirements.txt`
   - Without uv: `~/.olimpus/interagents/venv/bin/pip install -r <bin>/../requirements.txt`
5. **Tell the user**: "Installed in isolated venv at
   `~/.olimpus/interagents/venv`. Future `/interagents` commands
...
- Survives Python upgrades cleanly — just `rm -rf
  ~/.olimpus/interagents/venv` to reset.
...
- **Install uv** (`curl -LsSf https://astral.sh/uv/install.sh | sh`)
  and re-run `/interagents install-deps`. uv ships its own venv impl.
...
| User input                              | Bash                                              |
| :-------------------------------------- | :------------------------------------------------ |
| `/interagents auto-start status`      | `python3 <bin>/auto_start.py --status`            |
| `/interagents auto-start on`          | `python3 <bin>/auto_start.py --on`                |
| `/interagents auto-start off`         | `python3 <bin>/auto_start.py --off`               |

`on` = `when: "always"` (start at every session open).
`off` = `when: "on-skill-invoke:interagents"` (lazy: starts when the
user first invokes any `/interagents` command in a session). The
...
```
[interagents msg=q7r8 from="data-pipe" truncated=2097152] <first ~400 chars of text>
[interagents msg=q7r8 cont] full text 2.0 MB at ~/.olimpus/interagents/messages.log
```

The full payload is in `~/.olimpus/interagents/messages.log` as a
JSONL record. Fetch it with:

```
Bash("grep -F '<msg_id>' ~/.olimpus/interagents/messages.log | tail -1")
```

## Error notifications

If a monitor line begins with `[interagents]` (no `msg=`), it's an
